#!/usr/bin/env python3
"""Dedupe duplicate platform rows + flip statuses. Stored OUTSIDE the repo tree.
Usage: python3 ~/blackcatrobotics-repo/seeds-staging/dedupe_and_flip.py [--dry-run]
Run from TechMedix-standalone dir (reads .env.local)."""
...
for loser_slug, keeper_slug in PAIRS:
    ...
    if isinstance(lf,list) and isinstance(kf,list) and len(lf) > len(kf):
        loser, keeper = keeper, loser; lf, kf = kf, lf
        print(f"  (swapped: keeping {keeper['slug']})")
    print(f"KEEP {keeper['slug']} (FM={len(kf)})  DELETE {loser['slug']} (FM={len(lf)})")
    if DRY: continue
    if lf:
        r = req('PATCH', f"failure_modes?platform_id=eq.{loser['id']}", {"platform_id": keeper['id']})
        print(f"  repointed {len(r) if isinstance(r,list) else r} FMs")
    ...
    print(f"  deleted {loser['slug']}")

# flip hidden platforms to supported
for slug in ("veo-s1", "dewalt-dale", "lely-discovery-collector"):
    print(f"FLIP {slug} -> supported")
    if not DRY:
        r = req('PATCH', f"platforms?slug=eq.{slug}", {"techmedix_status": "supported"})
