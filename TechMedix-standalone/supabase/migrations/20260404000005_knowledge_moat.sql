-- ═══════════════════════════════════════════════════════════════════════════════
-- Fix: Add missing columns to existing suppliers table + create knowledge_moat tables
-- Run this AFTER 002_blackcat_os.sql but INSTEAD of the original knowledge_moat migration
-- ═══════════════════════════════════════════════════════════════════════════════

-- ── Fix suppliers table ──────────────────────────────────────────────────────────
ALTER TABLE suppliers ADD COLUMN IF NOT EXISTS component_types text[] DEFAULT '{}';
ALTER TABLE suppliers ADD COLUMN IF NOT EXISTS platforms_served text[] DEFAULT '{}';
ALTER TABLE suppliers ADD COLUMN IF NOT EXISTS unit_cost_usd numeric(10,2);
...
DO $$ BEGIN
  ALTER TABLE suppliers ADD CONSTRAINT suppliers_name_region_unique 
    UNIQUE (name, region);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
