    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        <div className="panel px-5 py-4">
          ...
          <p className="mt-1 text-3xl font-semibold tracking-[-0.04em] text-theme-primary">{evNodes.length}</p>
          <p className="mt-1 text-xs text-theme-40">Active vehicles</p>
        </div>
        <div className="panel px-5 py-4">
        ...
        <div className="panel px-5 py-4">
          ...
          <p className="mt-1 text-3xl font-semibold tracking-[-0.04em] text-theme-primary">
            {homeNodes.reduce((sum, n) => sum + (Number(n.metadata.solar_kwh) || 0), 0).toFixed(1)} kWh
          </p>
          <p className="mt-1 text-xs text-theme-40">Today across HABITAT nodes</p>
        </div>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <SurfaceCard title="EV fleet nodes" eyebrow="Registered vehicles">
          {evNodes.length === 0 ? (
            <p className="text-sm text-theme-40 py-4">No EV nodes registered.</p>
          ) : (
            <div className="space-y-3">
              {evNodes.map((node) => (
                <div key={node.id} className="rounded-[20px] border border-theme-5 bg-theme-2 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-theme-primary">{node.name}</p>
                      {node.metadata.location && (
                        <p className="text-xs text-theme-50">{String(node.metadata.location)}</p>
                      )}
                    </div>
                    <span className={`text-[0.65rem] uppercase tracking-[0.16em] px-2.5 py-1 rounded-full border font-medium ${
                      node.status === "online" || node.status === "idle"
                        ...
                        : "border-theme-10 bg-theme-4 text-theme-50"
                    }`}>
                      {node.status}
                    </span>
                  </div>
                  <div className="mt-3 grid grid-cols-3 gap-2">
                    {node.metadata.battery_pct !== undefined && (
                      <div>
                        <p className="text-[0.6rem] uppercase tracking-[0.14em] text-theme-30 mb-1">Battery</p>
                        <p className="text-sm font-semibold text-theme-primary">{String(node.metadata.battery_pct)}%</p>
                        <div className="mt-1 h-1 w-full rounded-full bg-theme-6">
                          <div
                    ...
                    {node.metadata.range_mi !== undefined && (
                      <div>
                        <p className="text-[0.6rem] uppercase tracking-[0.14em] text-theme-30 mb-1">Range</p>
                        <p className="text-sm font-semibold text-theme-primary">{String(node.metadata.range_mi)} mi</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div key={node.id} className="rounded-[20px] border border-theme-5 bg-theme-2 p-4">
                ...
                <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                  {node.metadata.power_kw !== undefined && (
                    <div>
                      <p className="text-[0.6rem] uppercase tracking-[0.14em] text-theme-30 mb-0.5">Capacity</p>
                      <p className="font-semibold text-theme-primary">{String(node.metadata.power_kw)} kW</p>
                    </div>
                  )}
                  {node.metadata.sessions_today !== undefined && (
                    <div>
                      <p className="text-[0.6rem] uppercase tracking-[0.14em] text-theme-30 mb-0.5">Sessions today</p>
                      <p className="font-semibold text-theme-primary">{String(node.metadata.sessions_today)}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="mt-4 rounded-[18px] border border-theme-5 bg-theme-2 p-4">
            ...
            {homeNodes.map((node) => (
              <div key={node.id} className="flex items-center justify-between py-2 border-b border-theme-5 last:border-0">
                <p className="text-xs text-theme-65">{node.name}</p>
      ...
      <SurfaceCard title="BlackCat Grid" eyebrow="Robot energy market">
        <div className="grid gap-4 sm:grid-cols-3 mb-6">
          <div className="rounded-[20px] border border-theme-5 bg-theme-2 p-5">
            <p className="text-sm font-semibold text-theme-60">Total Supply</p>
            <p className="mt-2 text-2xl font-semibold tracking-[-0.03em] text-moss">
              {grid ? `${grid.totals.supply_kwh} kWh` : "—"}
            </p>
            <p className="mt-1 text-xs text-theme-40">{grid?.supply.length ?? 0} robots supplying</p>
          </div>
          <div className="rounded-[20px] border border-theme-5 bg-theme-2 p-5">
            <p className="text-sm font-semibold text-theme-60">Total Demand</p>
            <p className="mt-2 text-2xl font-semibold tracking-[-0.03em] text-ember">
              {grid ? `${grid.totals.demand_kwh} kWh` : "—"}
            </p>
            <p className="mt-1 text-xs text-theme-40">{grid?.demand.length ?? 0} robots demanding</p>
          </div>
          <div className="rounded-[20px] border border-theme-5 bg-theme-2 p-5">
            <p className="text-sm font-semibold text-theme-60">Traded (session)</p>
            <p className="mt-2 text-2xl font-semibold tracking-[-0.03em] text-theme-primary">
              {grid ? `${grid.totals.total_traded_kwh} kWh` : "—"}
            </p>
            <p className="mt-1 text-xs text-theme-40">{grid?.totals.transaction_count ?? 0} transactions</p>
          </div>
        </div>

        {grid && grid.transactions.length > 0 ? (
          <div className="overflow-hidden rounded-[18px] border border-theme-5">
            <table className="min-w-full divide-y divide-black/5 text-left text-xs">
              <thead className="bg-theme-3 text-[10px] uppercase tracking-[0.2em] text-theme-40">
                <tr>
              ...
              <tbody className="divide-y divide-black/5 bg-white">
                {grid.transactions.slice(0, 8).map((tx) => (
                  <tr key={tx.id} className="hover:bg-theme-2">
                    <td className="px-4 py-3 text-theme-50">
                    ...
                    <td className="px-4 py-3 text-theme-60">${Number(tx.price_per_kwh).toFixed(4)}</td>
          ...
          <p className="text-sm text-theme-40">
      ...
      <div className="panel px-7 py-6">
        ...
        <div className="mt-2 flex items-center gap-3 flex-wrap">
          <h2 className="font-header text-[2rem] leading-none tracking-[-0.04em] text-theme-primary">
            Grid Infrastructure
          </h2>
        ...
        <p className="mt-3 text-sm leading-6 text-theme-65 max-w-2xl">
