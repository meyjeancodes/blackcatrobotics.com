...
function HealthBar({ label, score, max = 100 }: { label: string; score: number; max?: number }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="h-1.5 w-full rounded-full bg-theme-6 overflow-hidden">
        <div
...
function MotorCell({ label, motor }: { label: string; motor: DroneMotorHealth }) {
  ...
  return (
    <div className={`rounded-[16px] border border-theme-5 p-3 ${bg}`}>
      ...
      <div className="mt-2 space-y-1">
        <p className="text-[0.60rem] text-theme-35">RPM var: {motor.rpm_variance.toFixed(1)}</p>
        <p className="text-[0.60rem] text-theme-35">Vibe: {motor.vibration_level.toFixed(2)}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 panel p-5">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-theme-55 leading-relaxed max-w-xs">
        ...
        <div className="text-right shrink-0">
          <p className="font-ui text-[0.58rem] uppercase tracking-[0.18em] text-theme-35 mb-1.5">Recommended Action</p>
          ...
          <p className="mt-2 font-ui text-[0.58rem] uppercase tracking-[0.12em] text-theme-35">
      ...
      <div className="panel p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 pt-2 border-t border-theme-5">
          <div>
            <p className="text-xs text-theme-40 mb-1">Battery Trend</p>
            <p className="text-sm font-medium text-theme-70 capitalize">{data.battery_health.trend}</p>
          </div>
          {data.battery_health.cycle_count !== undefined && (
            <div>
              <p className="text-xs text-theme-40 mb-1">Cycle Count</p>
          ...
          <div>
            <p className="text-xs text-theme-40 mb-1">Signal avg RSSI</p>
            <p className="text-sm font-medium text-theme-70">{data.signal_health.avg_rssi} dBm</p>
          </div>
          <div>
            <p className="text-xs text-theme-40 mb-1">Packet Loss</p>
            <p className="text-sm font-medium text-theme-70">{data.signal_health.packet_loss_pct}%</p>
          </div>
        </div>
      </div>
        ...
        <div className="panel border-2 border-[#e8601e]/20 p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-theme-70 leading-relaxed">{data.care_refresh_recommendation.reasoning}</p>
              <p className="mt-1.5 font-ui text-[0.60rem] text-theme-40">
          ...
          {showMaintenance && (
            <div className="mt-4 space-y-2">
              {data.maintenance_items.map((item, i) => (
                <div key={i} className="flex items-center justify-between rounded-[14px] bg-theme-2 border border-theme-4 px-4 py-2.5">
                  <div>
                    <p className="text-sm text-theme-75">{item.item}</p>
                  ...
                  <p className="text-xs text-theme-40 shrink-0">{item.estimated_cost}</p>
                </div>
              ))}
            </div>
          )}
      ...
      {/* Generated at */}
      <p className="text-[0.60rem] font-ui text-theme-25 text-center">
