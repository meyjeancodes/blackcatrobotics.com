    ...
    <div className="panel p-6">
      ...
      <div className="flex items-center justify-between gap-4 mb-6 pb-5 border-b border-theme-5">
        <div>
          ...
          <div className="mt-2 flex items-center gap-3">
            <h2 className="font-header text-xl leading-tight text-theme-primary">March 2026</h2>
        ...
        <button className="font-ui text-[0.60rem] uppercase tracking-[0.14em] text-theme-45 border border-theme-12 px-4 py-2 rounded-full hover:bg-theme-4 transition-colors shrink-0">
          Export CSV
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ...
          { label: "Nodes",             value: `${NODES.length}`,                    cls: "text-theme-primary" },
        ].map((s) => (
          <div key={s.label} className="rounded-[18px] border border-theme-5 bg-theme-18 px-4 py-3">
            <p className="font-ui text-[0.56rem] uppercase tracking-[0.16em] text-theme-35 mb-1">{s.label}</p>
      ...
      <div className="hidden sm:block overflow-x-auto">
        <table className="w-full text-left" style={{ borderCollapse: "collapse" }}>
          <thead>
            <tr>
              {["Node", "Type", "Generated", "Consumed", "Traded", "Revenue", "Cost", "Net"].map((h) => (
                <th
                  key={h}
                  className="font-ui text-[0.56rem] uppercase tracking-[0.16em] text-theme-30 pb-3 pr-4 font-medium whitespace-nowrap"
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {NODES.map((node) => (
              <tr key={node.id} className="border-t border-theme-4 hover:bg-theme-15 transition-colors">
                <td className="py-3 pr-4">
                  <p className="text-sm font-semibold text-theme-primary">{node.name}</p>
                  <p className="text-[0.60rem] text-theme-38">{node.location}</p>
                </td>
                <td className="py-3 pr-4">
                  <span className="font-ui text-[0.58rem] uppercase tracking-[0.12em] text-theme-45">
                    {TYPE_LABEL[node.type]}
                  </span>
                </td>
                <td className="py-3 pr-4 font-ui text-xs text-theme-55">
            ...
            {/* Total row */}
            <tr
              className="border-t border-theme-8"
              style={{ background: "rgba(12,13,17,0.03)" }}
            >
              <td className="py-3 pr-4 font-semibold text-sm text-theme-primary">Total</td>
              <td className="py-3 pr-4" />
              <td className="py-3 pr-4" />
              <td className="py-3 pr-4" />
              <td className="py-3 pr-4" />
      ...
      <div className="sm:hidden space-y-2">
        {NODES.map((node) => {
          ...
          return (
            <div key={node.id} className="rounded-[18px] border border-theme-5 bg-theme-18 overflow-hidden">
              <button
                onClick={() => toggleRow(node.id)}
                className="w-full flex items-center justify-between gap-3 px-4 py-3"
              >
                <div className="text-left min-w-0">
                  <p className="text-sm font-semibold text-theme-primary truncate">{node.name}</p>
                  <p className="text-[0.60rem] text-theme-38">{TYPE_LABEL[node.type]}</p>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  {expanded ? <ChevronUp size={14} className="text-theme-35" /> : <ChevronDown size={14} className="text-theme-35" />}
                </div>
              </button>
              {expanded && (
                <div className="border-t border-theme-5 px-4 py-3 grid grid-cols-2 gap-2">
                  <div>
                    <p className="font-ui text-[0.55rem] uppercase tracking-[0.14em] text-theme-30 mb-0.5">Revenue</p>
                    <p className="text-sm font-semibold text-moss">${node.revenueThisMonth}</p>
                  </div>
                  <div>
                    <p className="font-ui text-[0.55rem] uppercase tracking-[0.14em] text-theme-30 mb-0.5">Cost</p>
      ...
      {/* Billing breakdown */}
      <div className="mt-6 pt-5 border-t border-theme-5 space-y-1.5 text-right">
        <p className="font-ui text-[0.60rem] text-theme-40" style={{ letterSpacing: "0.08em" }}>
          Platform fee (15%): ${platformFee.toLocaleString()}
        </p>
        <p className="font-ui text-[0.60rem] text-theme-40" style={{ letterSpacing: "0.08em" }}>
          Payable to BCR techs: $0 (no dispatch jobs this period)
        </p>
        <p className="font-ui text-[0.65rem] font-semibold text-theme-65" style={{ letterSpacing: "0.08em" }}>
          Net to operator: ${netOperator.toLocaleString()}
        </p>
      </div>

      {/* Coming 2027 note */}
      <div className="mt-5 pt-4 border-t border-theme-4 text-center">
        <p className="font-ui text-[0.58rem] text-theme-28" style={{ letterSpacing: "0.08em" }}>
