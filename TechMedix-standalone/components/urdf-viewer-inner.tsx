...
function UrdfRobot({ urdfUrl, onLoad, onError }: {
  urdfUrl: string;
  onLoad: () => void;
...
function UrdfScene({ urdfUrl, onError }: {
  urdfUrl: string;
  onError: (msg: string) => void;
}) {
...
export default function UrdfViewerInner({ urdfPath, label, height = 'h-[420px]' }: Props) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-xl border border-white/[0.06] bg-[#0c0c14] ${height}`}>
      ...
      <div className="h-full w-full">
        <UrdfScene urdfUrl={urdfPath} onError={setError} />
      </div>
    </div>
  );
}
