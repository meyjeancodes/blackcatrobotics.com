...
const STATUS_SEGMENTS: { key: NodeStatus; label: string; color: string }[] = [
  ...
  { key: "offline",     label: "Offline",     color: "bg-theme-20" },
];
  ...
  return (
    <section className="grid gap-6 xl:grid-cols-[0.45fr_0.8fr_0.75fr]">
      ...
      <div className="panel-elevated p-6 flex flex-col">
        ...
        <h2 className="mt-2 font-header text-xl leading-tight text-theme-primary">Live Score</h2>
        <div className="flex-1 flex flex-col items-center justify-center py-8">
          <p className={`font-header text-[88px] leading-none tabular-nums transition-all duration-500 ${scoreColor}`}>
            {score}
          </p>
          <p className="mt-2 font-ui text-[0.58rem] uppercase tracking-[0.24em] text-theme-35">
            out of 100
          </p>
          <div className="mt-6 w-full h-1.5 rounded-full bg-theme-5">
            <div
              className={`h-full rounded-full transition-all duration-700 ease-out ${barColor}`}
              style={{ width: `${score}%` }}
            />
          </div>
          <div className="mt-3.5 flex w-full justify-between font-ui text-[0.56rem] uppercase tracking-[0.16em] text-theme-30">
            <span>Critical</span>
            <span>Optimal</span>
          </div>
        </div>
        <div className="mt-2 rounded-[16px] bg-theme-25 px-4 py-3 font-ui text-[0.60rem] uppercase tracking-[0.16em] text-theme-35 text-center border border-theme-4">
      ...
      <div className="panel p-6">
        ...
        <h2 className="mt-2 font-header text-xl leading-tight text-theme-primary">Status Breakdown</h2>
            ...
            return (
              <div key={seg.key}>
                <div className="mb-1.5 flex items-center justify-between">
                  <span className="font-ui text-[0.63rem] uppercase tracking-[0.18em] text-theme-48">
                    {seg.label}
                  </span>
                  <span className="font-ui text-[0.63rem] text-theme-55 tabular-nums">
                    {count} / {dbTotal}
                  </span>
                </div>
        ...
        {/* Stacked bar */}
        <div className="mt-7">
          <p className="mb-2 font-ui text-[0.58rem] uppercase tracking-[0.20em] text-theme-30">
            Composite view
          </p>
          ...
          <div className="mt-3 flex flex-wrap gap-3">
            {STATUS_SEGMENTS.filter((s) => (dbCounts[s.key] ?? 0) > 0).map((seg) => (
              <div key={seg.key} className="flex items-center gap-1.5">
                ...
                <span className="font-ui text-[0.57rem] uppercase tracking-[0.16em] text-theme-42">
                  {seg.label}
                </span>
              </div>
            ))}
      ...
      <div className="panel p-6 flex flex-col">
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          <h2 className="font-header text-xl leading-tight text-theme-primary">System Feed</h2>
          <div className="flex items-center gap-1.5">
            <div
              className={`h-1.5 w-1.5 rounded-full ${connected ? "bg-moss" : "bg-theme-20"}`}
            />
          ...
          {realtimeFeed.map((entry) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-theme-58 leading-snug truncate mt-0.5">
                  {entry.message}
                </p>
              </div>
            </div>
